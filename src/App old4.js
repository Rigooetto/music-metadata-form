...
className = {`px-3 py-2 cursor-pointer ${idx === highlightedUpcIndex ? "bg-blue-100" : "hover:bg-blue-50"
    }`}
  >
    ...
    trackSuggestions.length > 0 && (
        <ul className="absolute z-10 mt-10 bg-white border border-gray-300 rounded-md w-full shadow-lg max-h-48 overflow-auto">
        {
            trackSuggestions.map((entry, idx) => (
                <li
          key= { idx }
          className = {`p-2 cursor-pointer ${idx === highlightedTrackIndex ? "bg-blue-100" : "hover:bg-blue-50"
                    }`}
          onMouseDown={() => {
    handleTrackChange(i, "primaryTitle", entry["Primary Title"] || "");
    handleTrackChange(i, "trackArtistName", entry["Track Artist Name"] || "");
    ...
    { activeArtistInputIndex === i && artistSuggestions.length > 0 && (
        <ul className="absolute z-10 mt-10 bg-white border border-gray-300 rounded-md w-full shadow-lg max-h-48 overflow-auto" >
        {
            artistSuggestions.map((name, idx) => (
                <li
        key= { idx }
        className = {`p-2 cursor-pointer ${idx === highlightedArtistIndex ? "bg-blue-100" : ""
                    }`}
onMouseDown = {() => {
    ...
    suggestions.length > 0 && (
        <ul className="absolute z-10 mt-10 bg-white border border-gray-300 rounded-md w-full shadow-lg max-h-48 overflow-auto" >
        {
            suggestions.map((sugg, idx) => (
                <li
      key= { idx }
      className = {`p-2 cursor-pointer hover:bg-blue-100 ${idx === highlightedIndex ? "bg-blue-100 font-medium" : ""
                    }`}
    onMouseDown = {() => {
        const updated = [...tracks];
        updated[i].composers[j] = {
    ...
    suggestions.length > 0 && (
        <ul className="absolute z-10 mt-10 bg-white border border-gray-300 rounded-md w-full shadow-lg max-h-48 overflow-auto" >
        {
            suggestions.map((sugg, idx) => (
                <li
          key= { idx }
          className = {`p-2 hover:bg-blue-100 cursor-pointer ${highlightedIndex === idx ? "bg-blue-100" : ""
                    }`}
onMouseDown = {() => {
    const updated = [...tracks];
    updated[i].composers[j] = {
...
className = {`px-3 py-2 cursor-pointer ${idx === highlightedIndex ? "bg-blue-100" : "hover:bg-blue-50"
    }`}
        >
    { s.publisher }
    </div>
